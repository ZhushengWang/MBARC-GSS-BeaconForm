...
namespace SPRL.Test
{
  public class SimCmds
  {
    ...
    private byte[] GetCommandArray(byte OpCode,UInt16 nLen)
    {
      byte[] CmdArray = new byte[nLen+5];
      
      CmdArray[0] = (byte)(0xC8 | ((OpCode & 0xE0) >> 5));
      CmdArray[1] = (byte)(((TCCounter & 0x0700) >> 8) | (byte)((OpCode & 0x1F) << 3));
      CmdArray[2] = (byte)(TCCounter & 0xFF);

      CmdArray[3] = (byte)((nLen >> 8) & 0xFF);
      CmdArray[4] = (byte)(nLen & 0xFF);  

      TCCounter++;
      return (CmdArray);
    }

    private void SendCommand(byte OpCode, byte[] ayData)
    {
      int nBytesToSend = ayData.Length;
      byte[] Cmd;
      int nCurrBytes;
      int nOffset = 0;
      ...
      else
      {
        ...
        {
          if (nBytesToSend > 1019)
          {
            nCurrBytes = 1019;
          }
          else
          {
            nCurrBytes = nBytesToSend;
          }
          Cmd = GetCommandArray(OpCode, (UInt16)nCurrBytes);

          for (int i = 0; i < nCurrBytes; i++)
          {
            Cmd[5 + i] = ayData[i + nOffset];
          }

          SendBytes(Cmd);

          nBytesToSend -= nCurrBytes;
          nOffset += nCurrBytes;
        }
      }
    }
    ...
    public void load_rabbit_fw(byte[] ayData)
    {
      SendCommand(0x97, ayData);
    }

    public void load_rabbit_fw(String sFName)
    {
      byte[] buffer;
      byte[] aySync;
      byte[] ayEmpty = new byte[0];  //Empty array for terminator

      try
      {
        FileStream binfile = new FileStream(sFName, FileMode.Open);
        int nLength = (int)binfile.Length;
        buffer = new byte[nLength];
        binfile.Read(buffer, 0, nLength);
        binfile.Close();
      }
      catch
      {
      ...
      SendCommand(0x97, aySync);
      SendCommand(0x97, buffer);
      SendCommand(0x97, ayEmpty);
    }
